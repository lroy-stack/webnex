...
const ClientDashboard = () => {
  ...
  // State for the onboarding alert
  const [showOnboardingAlert, setShowOnboardingAlert] = useState(!hasCompletedOnboarding);
  const [showOnboardingDialog, setShowOnboardingDialog] = useState(false);
  
  useEffect(() => {
    // Check if onboarding is skipped or completed
    const onboardingSkipped = localStorage.getItem("onboardingSkipped") === "true";
    
    // Show alert if onboarding is neither completed nor explicitly skipped
    setShowOnboardingAlert(!hasCompletedOnboarding && !onboardingSkipped);
  }, [hasCompletedOnboarding]);
  ...
  // Function to handle continuing onboarding
  const handleContinueOnboarding = () => {
    navigate("/onboarding");
  };

  // Function to permanently skip onboarding
  const handleSkipOnboarding = () => {
    setShowOnboardingDialog(true);
  };

  // Function to confirm skipping onboarding
  const confirmSkipOnboarding = () => {
    localStorage.setItem("onboardingSkipped", "true");
    setShowOnboardingAlert(false);
    setShowOnboardingDialog(false);
    toast.success("Has saltado la configuración inicial. Puedes completarla más tarde desde tu perfil.");
  };
      ...
      <Layout>
        <div className="container mx-auto py-8 px-4">
          ...
          <div className="relative">
            ...
            {showOnboardingAlert && (
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/30 dark:to-purple-950/30 border border-blue-200 dark:border-blue-800/50 rounded-lg p-4 mb-6 shadow-sm animate-fade-in">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div>
                    ...
                    <p className="text-blue-600 dark:text-blue-400 mt-1">
                      Configura tu perfil para aprovechar al máximo la plataforma.
                    </p>
                  </div>
        ...
        {/* Confirmation dialog for skipping onboarding */}
        <AlertDialog open={showOnboardingDialog} onOpenChange={setShowOnboardingDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>¿Estás seguro de que deseas omitir la configuración?</AlertDialogTitle>
              <AlertDialogDescription>
                Completar el proceso de configuración te ayudará a aprovechar al máximo la plataforma. Podrás acceder a la configuración más adelante desde tu perfil.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancelar</AlertDialogCancel>
              <AlertDialogAction onClick={confirmSkipOnboarding}>
