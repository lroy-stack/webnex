...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // The specific user ID to delete
    const userId = 'e0f7278a-29ea-411a-8976-fac97cdfe921';
    ...
    const { error: deletedAccountError } = await adminClient
      ...
      .insert({
        ...
        reason: 'Admin deletion - Requested removal',
    ...
    // Delete shopping cart items (requires deleting the cart first)
    ...
    // Delete project updates and milestones (requires getting project IDs first)
    ...
    // Delete chat messages (requires getting conversation IDs first)
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Usuario ${userData.user.email} (ID: ${userId}) eliminado exitosamente`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
    
  } catch (error) {
    console.error('Error en delete-specific-user:', error)
    return new Response(
